...
const DetailNilai: React.FC<DetailNilaiProps> = ({ onBack, category, user }) => {
    ...
    // Fetch grades for all subjects of this student
    const fetchAllGrades = async () => {
        if (!isSupabaseConfigured()) return;
        setLoading(true);
        try {
            const results: { [key: string]: any[] } = {};
            const semesterStr = selectedSemester === 'Ganjil' ? '1 (Ganjil)' : '2 (Genap)';

            // Fetch for each subject (this could be optimized, but let's keep it consistent)
            for (const sub of subjects) {
                const key = `grades_v2_${selectedClass}_${sub.name}_${semesterStr}`;
    ...
    const getStudentGrades = () => {
        return subjects.map(sub => {
            const semesterStr = selectedSemester === 'Ganjil' ? '1 (Ganjil)' : '2 (Genap)';
            const storageKey = `grades_v2_${selectedClass}_${sub.name}_${semesterStr}`;

            // Try cloud data first, then local
            const savedData = cloudGrades[storageKey] || (typeof window !== 'undefined' ? JSON.parse(localStorage.getItem(storageKey) || 'null') : null);
