...
import { ArrowLeft, BookOpen, ExternalLink, FileText, Plus, Save, X, Trash2, Send, FilePlus, HelpCircle, ChevronLeft } from 'lucide-react';
import {
    materiDataGlobal,
...
const MateriLatihanGuru: React.FC<MateriLatihanGuruProps> = ({ onBack, user }) => {
    ...
    return (
        <div className="bg-white rounded-[2rem] md:rounded-[2.5rem] shadow-sm border border-slate-200 overflow-hidden animate-in fade-in duration-300 flex flex-col h-full">
            {/* Header */}
            <div className="px-5 py-5 sm:p-8 border-b border-slate-100 flex items-center gap-3 md:gap-4 shrink-0 bg-gradient-to-r from-rose-50/50 to-red-50/30 sticky top-0 z-30">
                <button
                    onClick={onBack}
                    className="w-10 h-10 sm:w-12 sm:h-12 flex items-center justify-center bg-white shadow-md rounded-xl sm:rounded-2xl text-slate-400 hover:text-rose-600 hover:scale-110 transition-all active:scale-95 border border-slate-100"
                >
                ...
                <div className="min-w-0 flex-1">
                    ...
                    <p className="text-rose-600/60 text-[8px] sm:text-xs font-bold uppercase tracking-widest mt-0.5 whitespace-nowrap overflow-hidden text-ellipsis">Fasilitas pembelajaran digital guru & siswa.</p>
                </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 md:p-6 bg-slate-50/20">
                ...
                <div className="bg-gradient-to-br from-rose-600 via-rose-700 to-red-800 p-5 md:p-6 rounded-3xl text-white shadow-xl shadow-rose-100 flex items-center gap-4 border border-white/10 mb-6 font-sans">
                    <div className="p-3 bg-white/15 backdrop-blur-xl rounded-2xl shrink-0 hidden sm:block">
                        <BookOpen size={24} className="text-rose-100" />
                    </div>
                    <div>
                        <h3 className="font-black text-sm md:text-lg uppercase tracking-wide">Pusat Pembelajaran Digital</h3>
                        <p className="text-[10px] md:text-xs text-rose-100/90 leading-relaxed mt-0.5 italic">
                            Kelola materi ajar dan buat latihan soal interaktif untuk siswa Anda di sini.
                        </p>
                    </div>
                </div>
                ...
                <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm mb-6 flex items-center justify-between">
                    <div>
                        ...
                        <div className="bg-blue-50 border border-blue-100 rounded-xl px-4 py-2 text-sm font-bold text-blue-700">
                            <select
