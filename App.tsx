...
import { DataProvider } from './components/DashboardSuperAdmin/contexts/DataContext';
...
const App: React.FC = () => {
  ...
  const handleLogout = async () => {
    if (isSupabaseConfigured()) {
      try { await supabase.auth.signOut(); } catch (e) { logger.error("Logout error:", e); }
    }
    setIsLoggedIn(false);
    setUserRole('');
    setCurrentUser(null);
  ...
  if (isCheckingSession) {
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center space-y-4">
        ...
        <p className="text-slate-500 font-medium">Menghubungkan ke sistem...</p>
      </div>
    );
  }
  ...
  return (
    <ErrorBoundary>
      <Toaster />
      {!isLoggedIn ? (
        <Login onLogin={handleLogin} schoolName={schoolSettings.name} bannerImage={schoolSettings.bannerImage} logo={schoolSettings.logo} />
      ) : (
        <DataProvider>
          <AuthenticatedApp currentUser={currentUser} userRole={userRole} handleLogout={handleLogout} schoolSettings={schoolSettings} setSchoolSettings={setSchoolSettings} />
        </DataProvider>
      )}
    </ErrorBoundary>
  );
...
const AuthenticatedApp: React.FC<any> = ({ currentUser, userRole, handleLogout, schoolSettings, setSchoolSettings }) => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-8 text-center">
      <div className="w-20 h-20 bg-red-100 text-red-600 rounded-full flex items-center justify-center mb-6"><Bot size={40} /></div>
      <h1 className="text-2xl font-bold mb-2">Akses Terbatas</h1>
      <button onClick={handleLogout} className="px-8 py-3 bg-blue-600 text-white rounded-xl font-bold mt-8">Keluar & Masuk Kembali</button>
    </div>
  );
};
